...
async def get_customer_graph(
    ...
    try:
        ...
        # Get all edges connected to this customer
        # Priority: high-weight edges, transactions with large amounts
        edges_query = f"""
        SELECT *
        ...
        if depth > 1 and len(relationships) < limit_int:
            ...
            if first_degree_node_ids:
                ...
                # Add new edges and collect new node IDs
                new_node_ids = set()
                for idx, edge in enumerate(second_edges_result):
                    relationships.append(format_edge_for_cytoscape(edge, len(relationships) + idx))
