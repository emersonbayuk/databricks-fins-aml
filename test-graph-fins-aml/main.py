...
async def lifespan(app: FastAPI):
    ...
    try:
        logger.info(f"Initializing database service with warehouse_id: {config.DATABRICKS_WAREHOUSE_ID}")
        logger.info(f"Hostname: {config.DATABRICKS_HOSTNAME}")
        logger.info(f"Catalog: {config.CATALOG}")
        logger.info(f"Schema: {config.SCHEMA}")

        # Initialize database service (uses OAuth automatically)
        ...
        raise  # Raise the exception so we know what's wrong</
    finally:
        if db_service:
