...
# MAGIC
# MAGIC **Execution Time:** ~5-10 minutes
...
def generate_alerts(customers_df, accounts_df, transactions_df):
    """Generate AML alerts based on detection scenarios."""
    ...
    analysts = ["Sarah Chen", "Michael Rodriguez", "Emily Thompson", "David Park", "Jennifer Adams"]
    ...
    for customer in customers:
        ...
        if scenario == "normal":
            continue  # No alerts for normal customers
        ...
        for _ in range(num_alerts):
            ...
            alert = {
                ...
                "related_transactions": [],  # Would populate with actual transaction IDs
                "total_amount": round(random.uniform(10000, 500000), 2),
            }
            
            # Set resolution for closed alerts
...
alerts_df.write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.alerts")
...
cases_df.write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.cases")
...
alerts_enhanced.write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.alerts")
...
cases_enhanced.write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.cases")
...
# View 3: Alert Backlog by Scenario
spark.sql(f"""
...
views = ["v_executive_kpis", "v_analyst_performance", "v_alert_backlog_by_scenario", "v_analyst_queue", "v_customer_360"]
